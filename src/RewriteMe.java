...
public class RewriteMe {
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().flatMap(q -> q.getAllAnswers().stream()).anyMatch(p -> p.equals(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().flatMap(q -> q.getAllAnswers().stream()).filter(p -> p.equals(answerCandidate)).count();
    }
